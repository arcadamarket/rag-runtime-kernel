...
| **Version** | Specification **v3.2.7** | Runtime kernel **v0.4.46** |
...
> **On the two version numbers.** This repo tracks two things on separate version lines: the **specification** (the protocol the LLM follows — currently `v3.2.7`) and the **runtime kernel** (the Python engine that enforces it — currently `v0.4.46`). Tier 1 uses the spec alone; Tier 2 uses the runtime to enforce that spec.
