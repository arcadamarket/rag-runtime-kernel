...
| **Determinism** | As reliable as the model following instructions. | Deterministic state machine — formally verified (TLA+) and covered by 1,664 passing tests. |
...
| **Version** | Specification **v3.2.6** | Runtime kernel **v0.4.27** |
...
> **On the two version numbers.** This repo tracks two things on separate version lines: the **specification** (the protocol the LLM follows — currently `v3.2.6`) and the **runtime kernel** (the Python engine that enforces it — currently `v0.4.37`). Tier 1 uses the spec alone; Tier 2 uses the runtime to enforce that spec.
...
│   ├── __main__.py                            # CLI (init / configure / verify / health / serve / mcp / session / checkpoint / gc / graph / resolve / defer / render / report / note)
...
|---|---|---|---|
| Spec | **v3.2.6** | Released | **KA-11 inc3 — session-end claim-reconciliation pass baked into the spec (GOVERNANCE-DETERMINISM / KA-10 arc, TierB).** §50's session-end ritual gains a generic **claim-reconciliation pass** as its first step (reconcile → checkpoint → close → audit): before checkpoint, reconcile every published status-claim on the per-project `meta.reconciliation_surfaces` (TierC) against the tracked records — universalizing the formerly project-specific Rule 11 / INS-018 recurring pass so every fresh `init --spec` inherits it. Self-version 3.2.5 → 3.2.6; `session_end_protocol` rag-config updated + renumbered; `test_ka8` repointed to v3.2.6 + a new reconciliation-step test. Spec-only — no schema/runtime change (drift gate `268149294421` unchanged); `init --spec v3.2.6` seeds the reconciliation step + `reconciliation_surfaces` + Rule 11, order reconcile<checkpoint<close<audit, `verify` OK + `audit --strict` clean, suite 1,534 → 1,535 (+1). |
...
| Runtime | **v0.4.38** | Released | **KA-SCHEMA-MIGRATE — a governed, deployment-facing schema/version migration verb.** The kernel is deployed ONTO other projects; redeploying the pinned package does not move a deploy's `meta.schema_version` / `meta.policy_version`, so new code silently drives an old-shaped RAG and the only prior remedy was a forbidden hand-edit of another project's canonical state. New **`schema_migrate`** module + **`migrate`** CLI verb: a declared ladder of additive, idempotent steps whose terminal node IS the schema the kernel speaks (extending it is a data change, not a logic change). Direction is **never assumed** — every kernel-owned field is compared independently against the target's own meta, and a deployment that is AHEAD is refused, never silently downgraded (the live eBay clone runs `policy_version` 3.2.7 against this kernel's 3.2.6). Fail-loud on an unknown origin version, no-op when already current (no write, `.bak` untouched); project-owned state (`meta.rag_version`, `tracked_items` content, `operating_protocol`) is never touched (PRESERVE-IN-PLACE). Each migration appends a `meta.migrations` audit entry through the FIX-4 atomic byte-parity path. Runtime `__version__` 0.4.37 → 0.4.38, `__spec_version__` unchanged (3.2.6). **New module (19 → 20)**, health 21/21, drift gate `268149294421` unchanged, **1,861 → 1,911 tests** (+50). |
| Runtime | **v0.4.37** | Released | **REPORT-{RULE21-FIDELITY, BACKLOG-DEDUP, PRIORITY-COMPLETE} — the three report-content findings of the S153 transfer-surface audit, fixed together in the section-4 renderer.** The Rule 21 burn-down now always surfaces **P1** and the **active group** (an empty group emits an explicit `clear ✓` row, never silence) and marks the lowest-numbered group with active work `← ACTIVE`; active/deferred items are **referenced by id** so they no longer print twice alongside the flat Open/Blocked/Deferred lists (the itemized authority, Rule 16), while items resolved *this* session keep their title as the shipped signal; and the un-triaged catch-all carries an explicit `Unprioritized · needs a P-group` label instead of a silent `Unassigned`. Section 2 / the milestone cell now name the **live released build** (`release_ref`/`version`) rather than a stale newest-`RELEASE` row — folding the S154 cosmetic gap where the report read `RELEASE-v0.4.35` after v0.4.36 shipped. Runtime `__version__` 0.4.36 → 0.4.37, `__spec_version__` unchanged (3.2.6). No new module (still 19), health 20/20, drift gate `268149294421` unchanged, **1,859 → 1,861 tests** (+2). `drift_render` 1.2.0 → 1.3.0. |
| Runtime | **v0.4.36** | Released | **AUDIT-XFER-SURFACE-ATTEST (P1/F1) — the operator-facing report is bound to a verbatim attested file, closing the S152 chat-relay hole.** `session-end` already machine-rendered the deterministic, `report-attest`'d canonical report to stdout (S139 WIRE-CLOSE), but nothing persisted it or bound what the agent pastes into chat to that render — so a bare-count paraphrase with a hand-appended token slipped through at the S152 close (F1, the root-cause finding of the S153 transfer-surface audit). The close (`_drive_close` step 4/4) now also writes that exact attested text VERBATIM to a deterministic transfer-surface file, `RAG/AUDIT_CANONICAL_REPORT_<SID>.md`, and prints a pointer to present THAT FILE (verifiable with `report --verify <file>`) instead of retyping it. A new governed `transfer_surface_attestation` rule binds the operator-facing canonical report to the verbatim attested file only: Rule 22 (`report_render_attestation`) guards the RENDER; this rule binds the RELAY — the boundary the kernel previously could not see. Runtime `__version__` 0.4.35 → 0.4.36, `__spec_version__` unchanged (3.2.6). No new module (still 19), health 20/20, drift gate `268149294421` unchanged, **1,857 → 1,859 tests** (+2). |
| Runtime | **v0.4.35** | Released | **KA-INTENT-FIDELITY inc2 — the plan-vs-settled ID-bound audit completes the intent-fidelity milestone (inc1 shipped in v0.4.34).** A new `schemas.audit_plan_against_directive` + a fail-loud `intent-audit` CLI verb verify a NEW session's stated plan is faithful to the prior session's settled `next_session_directive`: **ID-binding** (cited decision ids must equal the directive's pinned `decision_ids` and each must resolve to a real `tracked_items` entry) plus a **normalized-exact restatement** of the directive text (whitespace/case-insensitive, never semantic; reuses inc1's `directive_matches`). The verb also **loads the SOURCE decisions** (resolves `decision_ids` → live tracked_item records, not the compressed handoff line), closing the other half of E-055 / the S146 drift. Deterministic, stdlib-only, zero-token. Runtime `__version__` 0.4.34 → 0.4.35, `__spec_version__` unchanged (3.2.6). No new module (still 19), health 20/20, drift gate `268149294421` unchanged, **1,841 → 1,857 tests** (+16). |
| Runtime | **v0.4.34** | Released | **REPORT-RENDER-ATTEST — the canonical status report is machine-attested against re-prosing.** `report` appends a `report-attest: sha256(<normalized body>)` trailer and a new `report --verify <file>` recomputes it and fails loud on any mismatch or absence; the governed `report_render_attestation` rule makes an unattested canonical report INVALID. Mechanizes Rule 12 for the in-chat / on-demand render path that S139 WIRE-CLOSE (session-end only) left unguarded — the gap behind E-060 (S136) and its recurrence E-062 (S149). Also ships **KA-INTENT-FIDELITY inc1** (already on main): a `next_session_directive` decision-of-record + a session-end handoff-persistence seal gate; increment 1 of 2 (plan-vs-settled ID-bound audit inc2 still to come). Runtime `__version__` 0.4.33 → 0.4.34, `__spec_version__` unchanged (3.2.6). No new module (still 19), health 20/20, drift gate `268149294421` unchanged, **1,813 → 1,841 tests** (+28: +19 inc1, +9 attest). |
| Runtime | **v0.4.33** | Released | **SECRETS-INGEST-GUARD (P1/G2) — declared-secret values refused at the ingest boundary, before commit.** The ingest-time complement to v0.4.32's audit-time `check_secrets_boundary`: KA-SECRETS-BOUNDARY was a *detective* control (a secret already in the RAG is a fail-loud audit finding), while `KernelApp.validate_secrets_ingest` is the *preventive* control — a proposal carrying a declared-secret value is rejected at `propose()` (the `BOOT → INGEST → VALIDATE → COMMIT` ingest transition) so the value never reaches `commit()`. Both paths share one source of truth, `drift_audit.collect_declared_secret_values` (`drift_audit` 1.13.0 → 1.14.0), so detection and prevention can never disagree on what counts as a declared secret (defaults `config/**`, `.env*`, `*.pem`/`*.key`, `credentials*`/`secrets*`; widenable via `meta.secret_paths`, never narrowable). Redaction-safe rejections (`sha256:<12>` + location, never the secret). Ships P1 as a tagged release (Rule 21 diversion trigger). Runtime `__version__` 0.4.32 → 0.4.33, `__spec_version__` unchanged (3.2.6). No new module (still 19), health 20/20, drift gate `268149294421` unchanged, **1,806 → 1,813 tests** (+7). Lane-A from the eBay S129 field audit (Rule 15). |
| Runtime | **v0.4.32** | Released | **P1 control-integrity release — the P1/G1+G2 guard batch plus the priority-group report render, deployed into the governance runtime.** Bundles five commits landed since v0.4.31: **ERRLOG-ID-GUARD (P1/G1)** — `check_errlog_id_coherence` makes ERROR_LOG error-id headings a fail-loud invariant (formally-verified `GUARD == I0 ∧ I1 ∧ I2`, catching malformed / reused / dangling ids); **KA-CS-PROSE-DRIFT (P1/G1)** — the E-043 freshness guard + refresh now cover **every** labeled `RUNTIME RELEASE` token in `current_status`, not just the leading one (`drift_audit` 1.11→1.12, `drift_store` 1.4→1.5); **KA-SECRETS-BOUNDARY (P1/G2)** — new `check_secrets_boundary` refuses declared-secret values appearing verbatim in the RAG, redaction-safe findings (`drift_audit` 1.12→1.13; Lane-A from the eBay S129 field audit, Rule 15); **REPORT-PRIORITY-GROUPS inc1** — structured per-item Rule 21 `priority_group` + guarded `set_priority` verb (`drift_control` 1.0→1.1, `drift_store` 1.5→1.6); **inc2** — `render_priority_burndown()` groups the backlog into P1..P5 + Unassigned as a subsection of report section 4 (`drift_render` 1.1→1.2). Runtime `__version__` 0.4.31 → 0.4.32, `__spec_version__` unchanged (3.2.6). No new module (still 19), health 20/20, drift gate `268149294421` unchanged, **1,733 → 1,806 tests** (+73). |
| Runtime | **v0.4.31** | Released | **REPORT-VERB-FIDELITY + REPORT-VERB-WIRE-CLOSE — the canonical status report renders faithfully and is machine-emitted at close.** FIDELITY: section 2 is a plan/actual `# \| Increment \| Plan \| Status \| RAG \| Commit-S` table from a new `TrackedItem.increments[]` on the active build (not a dump of all history); the milestone cell names the build shipped this session before "(no active milestone)"; and `guardgen` bakes `GUARDS_SELF_SHA256` + `verify_self()` so a deployed package with no `formal/RAGKernel.tla` self-verifies its guard integrity from baked provenance — removing the false-AMBER a genuinely-green released build used to read (unknown gates still pull to AMBER, Rule 14). WIRE-CLOSE: `session-end` machine-renders the deterministic canonical report verbatim as the mandated close artifact, so the closing report can never be hand-authored (the S136 close-drift root cause). Runtime `__version__` 0.4.30 → 0.4.31, `__spec_version__` unchanged (3.2.6). No new module (still 19), health 20/20, drift gate `268149294421` unchanged, **1,719 → 1,733 tests** (+14). |
| Runtime | **v0.4.30** | Released | **REPORT-VERB — the closing/transfer status report is now a deterministic kernel render, not a hand-authored prose block.** Rule 12 always required the 7-section canonical status report to be a *deterministic render of the RAG canonical fields*, but the render existed only as a hand-performed discipline, so a hand-assembled report could drift from a clean RAG (the transfer-drift gap behind the manual report-paste cross-check). New `report` verb (`drift_render.render_status_report` + `__main__.cmd_report`) renders all 7 sections as a pure projection: every fact is **structured** (`meta` / `tracked_items` / `inference_ledger`), **live-computed** (health, drift-gate sha, git HEAD, `.bak` parity), or an **explicit external arg** (`--context-pct`, `--tests`, `--released`, `--claims-ok`). It never scrapes `current_status` prose and never invents a value — an unknown fact renders `n/a` and can only pull the verdict toward AMBER, never a false GREEN (Rule 14). Objective R/A/G thresholds; backlog fully enumerated line-by-line (STRICT-OBEY); reuses `render_backlog_section` (no duplicate logic). Runtime `__version__` 0.4.29 → 0.4.30, `__spec_version__` unchanged (3.2.6). No new module (still 19), health 20/20, drift gate `268149294421` unchanged (no schema/WAL/TLA+ change), `DRIFT_RENDER_VERSION` 1.0.0 → 1.1.0, **1,693 → 1,719 tests** (+26). |
| Runtime | **v0.4.29** | Released | **KA-RECON-PROXIMITY + KA-RECON-DECLARE — the two follow-on gaps that blocked turning the close-time reconciliation on for this project, both closed.** **KA-RECON-PROXIMITY** (`drift_audit.check_repo_claim_reconciliation`) — the id-anchored §2 check paired a pending word with a RESOLVED id at *line* granularity, so a pending word anywhere on a long multi-clause paragraph line was read as the status of any RESOLVED id elsewhere on the line (a live false positive on this repo's own ROADMAP). It now segments each line on sentence / semicolon boundaries (`. ` / `; ` only — never dashes or table pipes) and requires the pending word and the id to co-occur in the **same sentence**, so it can only get more conservative and already-clean docs stay clean. **KA-RECON-DECLARE** (`configure`) — a governed `--reconciliation-docs-root PATH` flag sets `meta.reconciliation_docs_root` through the existing atomic `mirror_bak` writer (no forbidden hand-edit); `--context` is now optional and an explicit flag wins over any context-file value. Runtime `__version__` 0.4.28 → 0.4.29, `__spec_version__` unchanged (3.2.6). CLI/audit-only — no new module (19), health 20/20, drift gate `268149294421` unchanged (no schema/WAL/TLA+ change), `DRIFT_STORE_VERSION` unchanged (1.4.0), **1,680 → 1,693 tests** (+13). |
| Runtime | **v0.4.28** | Released | **KA-13 + KA-19 — the close-time published-doc reconciliation, now actually wired into `session-end` and no longer mis-firing on id substrings.** The spec (v3.2.6, KA-11 inc3) declared that the session close must reconcile every published status-claim against the tracked records, but the runtime close ran its step-3 audit with `docs_root=None`, so the Rule 11 doc reconciliation stayed dormant (RECONCILE-PASS-RECURRING). **KA-13** (`session-end` / `session-resume`) resolves a `docs_root` for the close audit with a back-compatible precedence — `--no-reconcile` > `--docs-root PATH` > `meta.reconciliation_docs_root` > skip — so a project that declares its surface root reconciles its published docs against the live canonical facts as part of the fail-loud close, while an un-migrated RAG closes byte-for-byte as before. **KA-19** fixes the id-anchored status check: it matched a tracked id with a bare substring (`rid in ln`), so a RESOLVED `FIX-1` was reported "still pending" against a line naming only `FIX-12`; the id is now matched at token boundaries (`(?<![\w-])…(?![\w-])`) so no digit/hyphen extension can trigger a shorter id, while an exact mention still fires. **RECONCILE-PASS-RECURRING** — the recurring repo-claim↔reality↔record pass is mechanized by the KA-13 close wiring. Runtime `__version__` 0.4.27 → 0.4.28, `__spec_version__` unchanged (3.2.6). CLI/close/audit-only — no new module (19), health 20/20, drift gate `268149294421` unchanged (no schema/WAL/TLA+ change), `DRIFT_STORE_VERSION` unchanged (1.4.0), **1,664 → 1,680 tests** (+16). |
| Runtime | **v0.4.27** | Released | **KA-CS-REFRESH — governed `refresh-current-status` verb: the atomic repair half of the E-043 freshness guard.** `current_status` denormalizes two facts whose authority lives OUTSIDE the RAG — the runtime `__version__` and the published git HEAD — and the E-043 guard fails loud when the narrative drifts from them, but there was no governed way to *repair* that drift: a mid-session dev commit left `current_status` stale and the only fix was a forbidden hand-edit (cost a manual reconcile at S116 + S127). New **`refresh-current-status`** (`drift_store` / CLI) re-stamps the runtime-version token (`current_status.rag_kernel_version` ← live `rag_kernel.__version__`) and the git HEAD (`github_repo`'s "LATEST COMMIT &lt;sha&gt;" ← auto-resolved worktree HEAD), plus optionally the `unit_tests` count (`--tests`, never fabricated). Backed by new `drift_store.compute_current_status_refresh` (pure) / `refresh_current_status_file` (atomic), reusing the FIX-4 `tmp → verify → .bak → rename` byte-parity path so HOT↔.bak parity holds by construction. Idempotent (no-op when fresh), `--dry-run` shows the old→new diff, `--strict` fails loud on a missing token; re-stamps only the machine-fact token, leaving narrative to the agent. The guard's leading-token regexes moved **down** into `drift_store` as the single source of truth (`drift_audit` re-exports them), so detection and repair can never disagree. Runtime `__version__` 0.4.26 → 0.4.27, `__spec_version__` unchanged (3.2.6). CLI/store/audit-only — no new module (19), health 20/20, drift gate `268149294421` unchanged (no schema/WAL/TLA+ change), `DRIFT_STORE_VERSION` 1.3.0 → 1.4.0, **1,639 → 1,664 tests** (+25). |
| Runtime | **v0.4.26** | Released | **T1 GATE — three governance-hardening fixes (KA-CTX-RAGFLAG + KA-CKPT-PARITY-GATE + KA-18).** **KA-CTX-RAGFLAG** — `context set/get/list` now tolerates a `--rag <file>` path and routes the partition to the file's parent dir (it previously expected a directory and silently mis-routed the natural file invocation), matching every other verb's `--rag` semantics. **KA-CKPT-PARITY-GATE** (E-049) — `checkpoint` now auto-renders the legacy `open_tasks` / `deferred_items` arrays from the canonical `tracked_items` at seal so render-parity holds by construction, plus a defensive fail-loud if a stale render is detected at the gate (the `audit` render==canonical invariant enforced at the checkpoint boundary, not only session-end). **KA-18** (E-044/E-045) — `checkpoint` refuses to run without an open session log so the mechanized `session-start` must precede it; CLI default ON, `--no-require-session-log` the explicit audited bypass. Runtime `__version__` 0.4.25 → 0.4.26, `__spec_version__` unchanged (3.2.6). CLI/checkpoint/context-only — no new module (19), health 20/20, drift gate `268149294421` unchanged (no schema/WAL/TLA+ change), `DRIFT_STORE_VERSION` unchanged (1.3.0), **1,623 → 1,639 tests** (+16). |
| Runtime | **v0.4.25** | Released | **KA-CUTOVER-GATE — record-coverage cutover gate counts only non-retired records + governed `un-add` verb.** Fixes a deadlock where a mis-`add` of a forensic-kind item (`ERROR`/`INFERENCE`) latched the per-kind cutover gate ON with no way to clear it: `check_record_coverage` counted a kind as migrated regardless of status, and there was no un-add path — discard/supersede leave `kind` intact, so the status-blind gate stayed latched. Now `drift_control.RETIRED_STATUSES` (`{SUPERSEDED, DISCARDED}`, a strict subset of `TERMINAL_STATUSES`; `RESOLVED` still counts) carves retired items out of the per-kind gate, and a guarded, atomic `un-add` (`TrackedItemStore.remove` / `drift_store.remove_item_file` / CLI `un-add`) removes a PRISTINE (empty-history) mis-`add` outright — fail-loud on an unknown id or a historied (real) item, so real lifecycle history is protected. Recovers a mis-`add` without a hand-edit (the E-037/E-040 drift). Runtime `__version__` 0.4.24 → 0.4.25, `__spec_version__` unchanged (3.2.6). CLI/store/audit-only — no new module (19), health 20/20, drift gate `268149294421` unchanged (no schema/WAL/TLA+ change), `DRIFT_STORE_VERSION` 1.2.0 → 1.3.0, **1,623 tests** (+17). |
| Runtime | **v0.4.24** | Released | **UPDATE-RULE-VERB — governed re-set of an existing `operating_protocol` rule through the guarded atomic store.** The counterpart to `add-rule` (which only *appends* a new rule, fail-loud on an existing key), with the inverse default and two capabilities `add-rule` lacks. `update-rule` (`71befae`) re-sets a rule that **must already exist** (UPDATE default; `--create` to add instead), and adds **`--json`** (re-set a structured rule like `tool_hierarchy` with a dict/list value wholesale, not only a string) and **`--subkey`** (trim/re-set one sub-entry of a dict rule at a time). Backed by new `drift_store.set_operating_protocol_rule` / `set_operating_protocol_rule_file`, reusing the FIX-4 atomic `tmp → verify → .bak → rename` byte-parity `.bak`-mirror path so an `operating_protocol` mutation keeps HOT↔.bak parity by construction. Unblocks the `tool_hierarchy` dict-trim — the last piece of **RAG-LEAN-PROSE**. Runtime `__version__` 0.4.23 → 0.4.24, `__spec_version__` unchanged (3.2.6). CLI/store-only — no new module (19), health 20/20, drift gate `268149294421` unchanged (no schema/WAL/TLA+ change), `DRIFT_STORE_VERSION` unchanged (1.2.0), **1,603 tests** (+34). |
| Runtime | **v0.4.23** | Released | **KA-14 + KA-16 + KA-17 — the session-resilience arc (bundled runtime release).** Packages the three runtime increments merged since v0.4.22. **KA-16** (`aa34e97`) — atomic, resumable session close: a `session_close` marker sets `transfer_ready` only after checkpoint + idempotent ERROR_LOG fold + logger close + audit all pass, and `session-resume` finishes an interrupted close (+12 tests). **KA-14** (`e34691b`) — session-start rule-load attestation gate: a two-phase token-attested start (`BOOT → RULES_LOADED(attested) → READY`) closing the fresh-deploy unloaded-rules root cause (+15 tests). **KA-17** (`dc5f0c0`) — a declared, single-sourced supported-Python matrix (3.12–3.14) with a `doctor` ENV check, reconciling the former unsubstantiated `>=3.10` claim across manifest + README + 4 docs (+7 tests). Runtime `__version__` 0.4.22 → 0.4.23, `__spec_version__` unchanged (3.2.6). The companion token-economy / context-emission doctrine is live in `operating_protocol`; the INIT-spec v3.2.7 seeding shipped (spec v3.2.6 → v3.2.7, commit `839e77b`), and a `python314_pip` accuracy fix is tracked separately (`KA-PY314-ACCURACY`) for a later spec bump. No new module (19), health 20/20, drift gate `268149294421` unchanged (no schema/WAL/TLA+ change), **1,569 tests**. |
| Runtime | **v0.4.22** | Released | **KA-11 inc4 — TierC kernel reconciliation-surface manifest population + docs reconcile; the runtime release bundling KA-11 inc1–4 (GOVERNANCE-DETERMINISM / KA-10 arc).** inc1–2 added the per-project `meta.reconciliation_surfaces` manifest schema + reader, wiring it through `audit_file` / `audit --docs-root` — replacing the hardcoded kernel-specific doc list with a per-project declaration (defaulting to README / CHANGELOG / ROADMAP) so the auditor is no longer kernel-repo-specific; inc3 added the TierB INIT-spec session-end claim-reconciliation pass (spec v3.2.5 → v3.2.6); inc4 populates the kernel's own TierC manifest (README / CHANGELOG / docs/ROADMAP) and reconciles the published docs against the live canonical facts (`__version__`, module count, drift-gate sha). Runtime `__version__` 0.4.21 → 0.4.22, `__spec_version__` 3.2.5 → 3.2.6. Eleventh runtime increment of the KA-10 GOVERNANCE-DETERMINISM initiative; **completes KA-11** and the Track A kernel-hardening arc surfaced by the eBay S0 audit, unblocking the eBay re-init (INS-047). No new module (19), health 20/20, drift gate `268149294421` unchanged (no schema/WAL/TLA+ change), **1,535 tests**. |
