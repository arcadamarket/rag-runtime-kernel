...
_Four gates that existed and were never called, and the discovery that the drift-checker everyone pointed at could not travel to a second machine._

- **`PROJECTION-DRIFT-UNGATED` — the transport allowlist is now GATED, not merely checkable.** S197 declared `operating_protocol.transport_allowlist` as the authority and rendered `.claude/transport_allowlist.json` as a projection carrying the sha256 of the rule text — then called the comparison from nowhere. Drift was DETECTABLE and NOT GATED, and the rule's first draft asserted "audit fails on drift", which was false when written. `drift_audit.check_transport_projection` now runs inside `audit_hot` on every audit, from the HOT dict already in memory: no second RAG read, no lock, no subprocess. Divergence, a hand-edited projection, an unparseable projection, and a **declared rule with no projection rendered at all** are each an ERROR. A test asserts `assert_clean` RAISES, so the severity cannot be softened without a test failing.
- **`RENDERER-UNTRACKED-S197` — a gate that lives in one working copy is a local habit.** The renderer that `PROJECTION-DRIFT-UNGATED` told its implementer to shell out to existed ONLY in this deployment's `RAG/tools/`: untracked by git, absent from the worktree, outside the test gate, invisible to every clone. The logic moved into `rag_kernel/transport_projection.py` — one implementation, two callers, one suite — and `tools/render_transport_allowlist.py` is now argument parsing over it. A test asserts the module imports from `rag_kernel`, so it cannot migrate back out.
- **`PHANTOM-SESSION-ID` — AUTO-SID-DERIVE refuses malformed ids instead of inheriting them.** `_derive_next_sid` matched `(\d+)$` and incremented whatever it found, so a millisecond epoch parked in `meta.written_by_session` derived cleanly into `S1786488555314` and then *looked* governed — it reached a session log and three WAL entries before the close audit refused. The guard now requires an alphabetic prefix and bounds the counter at nine digits, which excludes both epoch widths (10-digit seconds, 13-digit milliseconds). **Deliberately wider than the ledger item asked:** the item said the literal `S<n>` shape, which would have broken `SESS0099 -> SESS0100` and every clone that numbers sessions with its own prefix. The prefix was never the defect; the missing length ceiling was.
- **`WAITFOR-LEADING-DASH-ARGV` — the anti-polling primitive can finally express the token most worth waiting for.** `wait-for --contains "--attest"` died in argparse, because a dash-leading token parses as a flag. The single most useful thing to wait for at boot is the attestation line, whose token *is* `--attest`; a wait verb that cannot say it pushes the agent straight back to the banned polling, which is exactly what happened at the S198 boot. Folded at the argv layer; a test asserts the unfolded form still raises, so the fold is load-bearing rather than decorative.
- **Formal verification extended to the new gates, and made repeatable.** `SessionIdShape.tla` proves the shape guard refuses both epoch widths while admitting every real counter and clone prefix, squeezing `MaxDigits` from both sides so the bound cannot be widened later without breaking a proof. `TransportProjectionGate.tla` proves the audit clause errors on exactly the divergent deployments and self-skips undeclared ones, stated as a biconditional. Both ship with counterfactual configs that TLC must REFUTE. `formal/run_tlc.sh` runs all 21 configs in ~52 seconds: TLC scratch on local disk (from the `/mnt/c` 9p mount, RAGKernel's 2-minute run took over twenty and looked hung), one directory per config, and it refuses a vacuous zero-config pass.
- **`GITIGNORE-SWALLOWS-PROJECT-SCRIPTS` (open) — a blanket `*.sh` rule silently un-tracked the runner above.** `git add -A` took it, `git status` reported "working tree clean", and a verification tool sat untracked on one machine — `RENDERER-UNTRACKED-S197` recurring inside the session that resolved it. The instance is fixed by an explicit negation; the class is not. Owed: an audit clause that ERRORs when a registered `baked_asset` path is git-ignored — the asset registry already knows every file the project claims to own.
- Suite 2,714 -> **2,758** (measured by the `tests` verb, not typed). New modules `tests/test_transport_projection_gate.py`, `tests/test_phantom_session_id_refusal.py`, `tests/test_waitfor_dash_token.py`.

## [v0.4.58] - 2026-08-11

_Documented in arrears by S198: v0.4.58 shipped without a CHANGELOG entry, which is the same defect class as a handoff nobody gates._

- **`rag_wait` — an agent-side blocking read over MCP.** `wait-for` blocks inside a tmux pane, so from the agent's side a finished wait is indistinguishable from a running job and the only way to learn otherwise is to query the result — i.e. to poll. `rag_wait` moves the wait into the MCP transport, where the call itself blocks and returns once.
- **Transport allowlist inverted (E-133).** `operating_protocol.transport_allowlist` declares which agent-facing transports may run, and the PreToolUse hook enforces it by prefix.
- **Hook scope ruled down to boundary-only gates.** A hook earns its cost only where a non-deterministic actor chooses an action the kernel cannot observe. Everything a verb can enforce belongs in the verb: deterministic, unit-tested, fails closed, travels to clones. `prompt-frame` / `stop-seal` / `session-boot` were wired and cut in the same session under that test, and a test now FAILS if they are re-added.
- **Orphan-session regression, caused and fixed in the same session.** `cmd_mcp` called `app.boot()` eagerly with no `--session-id`, so every launch of the MCP server minted a timestamp session, opened a log, took the lock and wrote WAL entries from a fresh seq-1 allocator: six phantom sessions and a non-monotonic WAL from one bug. Boot is now lazy and refused without an explicit id, pinned by 11 tests including one that fails if `rag_wait` ever starts needing a boot.

## [v0.4.57] - 2026-08-11

_A check whose declared question is wider than the set it actually inspects. Three of them, one patch — and then the first enforcement that does not depend on the agent remembering anything._

- **`AGENDA-KIND-SCOPE` — the boot agenda can finally name an error.** `render_priority_actions` borrowed `BACKLOG_KINDS`, which deliberately excludes `kind=ERROR`, so a P1 error was structurally unnameable on the surface every session reads first. Thirteen P1 items existed; the briefing said seven, silently, and every downstream count inherited the omission and still called itself complete. The agenda now has its own scope, `AGENDA_KINDS = BACKLOG_KINDS + ERROR`. The legacy task arrays keep `BACKLOG_KINDS` and stay byte-identical, so the E-040 render-parity check is untouched. (E-129)
- **`LIVE-SET-SCOPE` — the triage check now sees the whole live set.** `no unprioritized live items` scoped to OPEN and DEFERRED and could not see IN_PROGRESS, so an item sat live with no priority while the check that exists to guarantee triage reported clean. Its name promised more than it measured. (E-131)
- **`INSPECTED-COUNT-DISCLOSURE` — every completeness surface now reports what it looked at.** The boot briefing enumerates the agenda from `tracked_items` rather than parsing the persisted projection, and states the denominator: how many live items it walked, out of how many tracked, over which kinds. It also WARNS at boot when the persisted `priority_actions` omits a live P1 instead of leaving that to the auditor. A verdict with no stated denominator is what let a truncated terminal listing pass for a whole backlog. (E-130 prevention, owed under `PLAN-FEASIBILITY-GATE`)
- **`HOOK-ENFORCEMENT-LAYER` (new module `rag_kernel/hook_guard.py`) — process rules that REFUSE instead of remind.** Every recurring defect in this log is a rule the agent had loaded, could recite, and broke anyway: E-081 recurred as E-116 and again as E-128. The rules were never forgotten, they were unenforceable — the agent was both the governed party and the check. Four gates now fire from `.claude/settings.json` before the call happens: `poll` refuses a repeat query against a running command id inside the cooldown and names `wait-for` as the alternative; `sandbox-state` refuses a sandbox shell that names canonical state (E-071); `canonical-read` refuses a direct read or hand-edit of `RAG_MASTER.json` (boot rule 1, tool_contract clause 1); `deploy-parity` reports deployed-vs-committed drift at the moment of the edit rather than at the seal. The decision function is pure and unit-tested, so the policy is checkable without a live agent.
- **The layer's one hope is declared and measured.** A hook process that cannot start cannot refuse anything, and `hook_guard` deliberately FAILS OPEN — loudly, on stderr — because a crashing monitor that denies every `Read` bricks the session it protects. `hook-guard --selftest` drives every gate through a known-bad payload and asserts the refusal, so "the hooks are live" is a command with an exit code instead of an assumption.
- **`E-132` (open) — `session-end --handoff` asserts backlog facts that no gate checks.** Found at the S195 boot reconciliation: the S194 handoff stated P1 was 13 items and named `E-131` among them; canonical state said 12, with `E-131` at P2 — created by that same session and never promoted. Two sibling instances in the same sweep: a reported `SUPERSEDED` transition the verb had actually REFUSED, and a "cleared" note contradiction that merely swapped one matched word for another. Same family as E-129 and E-131, landing in the one artifact the successor trusts most.
- Suite 2,591 -> **2,637** (measured, not typed). New module `tests/test_hook_enforcement_layer.py` (39 tests), including a case asserting that `./RAG_MASTER.json` cannot step around the basename guard, and one that refuses to let `.claude/settings.json` name a gate that does not exist.

## [v0.4.56] - 2026-08-10

_The interval guards. A gate that is correct at the moment it runs, and silent about every moment after, is how S191 sealed green over an uncommitted kernel — in the session it spent fixing exactly that._

- **`SEAL-INTERVAL-RECHECK` — four probes re-run as the last act before `transfer_ready`.** `CLOSE-TESTGATE-STALE-BLOCKS` (E-115) evaluates the test gate when the close BEGINS and was correct at that instant; it says nothing about the interval between that instant and the write of the marker. S191 passed it, then edited `__main__.py` while the close was still running, and sealed COMPLETE over code that existed in no commit. The probes now run again with nothing between them and the seal: gate green at the live kernel HEAD, worktree committed, deployment in lockstep, no orphaned E-numbers. No `--force`: everything reachable by force is reachable by fixing the tree, and a seal is the one artifact the next session cannot re-derive. (E-123)
- **`BOOT-TESTGATE-STALE-BLOCKS` — the boot finally asks.** S191 told the operator in prose that the S192 boot would refuse on a stale test gate. It would not have: the staleness check lived only in `grand_audit` axis 2, which the boot does not run. `meta.test_gate` held `STALE` in canonical state the entire time and nothing on the startup path consulted it. Now step 5 of the carry-forward gate. (E-124)
- **`ORPHAN-ENUM-BLOCKS` — a cited `E-nnn` with no tracked item is a fabricated identifier.** S191 wrote E-111, E-114 and E-115 into `ERROR_LOG.md`, cited them in source comments and commit messages, and never banked any of them — three ids that pointed at nothing for a full session, reported as banked. Prose does not transfer between sessions; tracked items do. The boot and the seal both refuse on an unbacked citation, naming the `add` command as the repair. (E-125)
- **Kernel-copy lockstep now runs at the seal, not only at gate-time.** `check_kernel_copy_lockstep` (S188) already compared the deployed and tested trees through `audit_file`; what it never covered was the interval. `_probe_deploy_parity` delegates to it — after first duplicating it, which is banked as its own defect. (E-126)
- **All four are ASSERTED state and are never auto-repaired.** Re-running a suite, banking an id, committing a tree and syncing a deployment are acts with content; performing them silently at boot would convert a refusal into a cover-up, which is what `GATE-AUTO-RECONCILE` exists to forbid.
- **Two defects found in the guards themselves, by the guards, before shipping.** `FAIL-OPEN-ON-UNRESOLVABLE`: a relative `rag_path` made every probe return "no findings" against a RAG that was provably stale *and* provably dirty, because `verdict` compares only when it is GIVEN a live head. `HEAD-PROBE-DISAGREEMENT`: `_resolve_git_head` walks `current_status.git_worktree_path` while the worktree probes resolve `meta.reconciliation_docs_root` — two probes grading "the kernel" against two different repositories. All four are now pinned to one tree via `_live_kernel_head`, and refuse when they cannot look.
- **`E-127` (open) — `reuse-check` returned CLEAR for a capability the kernel already had.** It searches `baked_assets`, i.e. registered files, and answers "is there a registered asset for this"; Rule 25's question is "does this capability exist anywhere in the kernel". A compliant check produced a violation. Owed: search function names, docstring first lines and the CHANGELOG, and state the scope actually searched instead of "safe to author".
- **`operating_protocol.interval_guards`** added, so the discipline loads in the rule digest at every boot alongside the gates that enforce it.
- Suite 2,569 -> **2,591**. New module `tests/test_interval_guards.py` (22 tests), including a case that asserts `git status` is CLEAN before checking deploy parity — to prove the guard sees what git structurally cannot.
