...
## v0.4.47 / spec v3.2.8 — Released (2026-07-26)

SPEC-PROMOTION-DRIFT + RULE19-PIN-REFRESH, gated as one unit. **SPEC-PROMOTION-DRIFT (E-063, spec-promotion half)** promotes the boot-integrity spine from this project's `operating_protocol` into the universal spec as `INIT_UNIVERSAL_RUNTIME_KERNEL_v3.2.8.md`, so a fresh `init --spec` deploy *inherits* both boot-boundary guards instead of re-authoring them: §50 gains **BOOT-GUARD-FIRST-ACTION (KA-20)** — `session-start` is the VERY FIRST action of every session, the canonical RAG is never read directly to LOAD or REPORT boot state (the command renders a complete deterministic boot-state briefing), the start is two-phase and token-attested (`BOOT → RULES_LOADED(attested) → READY`, KA-14) with a `boot_guard` first-action marker, the session id is optional from v0.4.46 (AUTO-SID-DERIVE, zero-read boot), and the rule states its scope honestly (remove the trigger, record the proof — it does not claim to observe an out-of-band read) — and **CLOSE-SEAL-ENFORCE (KA-21)** — the carry-forward gate refuses to open over an UNSEALED predecessor (sealed = `transfer_ready` **and** the attested `AUDIT_CANONICAL_REPORT_<sid>.md` present), names its recoveries (`session-resume` / `session-end <prior sid>`), leaves legacy RAGs untouched, keeps `--force` as a sanctioned logged override. The session-end ritual gains the matching explicit **seal** step (5) and renumbers to five; both `session_start_protocol` and `session_end_protocol` rag-config strings encode the guards (+6 tests). **RULE19-PIN-REFRESH (F3 residual)** adds `drift_audit.check_governance_pin_provenance`, binding the Rule 19 `governance_runtime` pin to the live `rag_kernel.__version__` authority — the rule that declares WHICH runtime enforces the rules had itself drifted silently (`v0.4.23/c140137` in the rule against code at `v0.4.45`, twenty-two releases, every audit clean). A declared pin that disagrees is an ERROR; a pin with no release provenance, or an unparseable pin claim, is a WARNING rather than a silent pass; history tokens are deliberately not anchored, and the pinned commit is deliberately NOT compared to git HEAD (the pin names the release commit, by design behind HEAD). Always-on in `audit_hot` (+12 tests). Also releases the S174 **FV-GATE-RETROVERIFY** proofs (6/6 TLC, 6/6 naive refutations). Runtime `__version__` 0.4.46 → 0.4.47, `__spec_version__` 3.2.7 → **3.2.8** (a byte change to the deployed backbone, so a full runtime release, not a spec-only bump); no new capability module (still 20), health 21/21, drift gate `268149294421` unchanged; full suite 2,032 → 2,050 green (+18).

---

## v0.4.46 — Released (2026-07-26)

AUTO-SID-DERIVE (zero-read boot) + `doctor --recover`. `session-start` now derives the next session id itself so the opening ritual needs no direct RAG read at boot (reinforcing BOOT-GUARD-FIRST-ACTION / KA-20), and `doctor --recover` adds a `.bak`/COLD/WAL/rebuild recovery advisor for a corrupted or partially-written RAG. Ships the `formal_verification_gate` governance rule. Runtime `__version__` 0.4.45 → 0.4.46, `__spec_version__` unchanged (3.2.7); no new capability module (still 20), health 21/21, drift gate `268149294421` unchanged; full suite 2,021 → 2,032 green (+11); TLC `RAGKernel.cfg` full reachable state space clean (505,560 distinct states, 0 violations). Tagged `runtime-v0.4.46` (commit 8af6ed6), pushed to origin. **Follow-on (S174, released in v0.4.47):** FV-GATE-RETROVERIFY added `formal/` characterization proofs for all six S144–S172 hardenings (secrets-ingest, KA-20, KA-21, intent-fidelity, bootmap-root-pin, schema-migrate) — 6/6 TLC proofs pass, 6/6 naive refutations fail as designed.
