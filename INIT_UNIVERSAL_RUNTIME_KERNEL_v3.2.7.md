# INIT_UNIVERSAL_RUNTIME_KERNEL_v3.2.7
...
> Supersedes: all prior versions through v3.2.6.
>
...
When the wrapper is present, `rag_kernel session-end` runs checkpoint → close → audit in that order (any step's failure aborts the rest), and its audit step mechanically verifies the declared `reconciliation_surfaces`; the claim-reconciliation pass itself (the record ↔ surface mapping and any doc fix-ups) is performed by the agent BEFORE invoking `session-end`, until a future increment wires the pass into the command as a distinct first step. AUTONOMOUS mode performs all four steps manually, in order, halting on any failure.
