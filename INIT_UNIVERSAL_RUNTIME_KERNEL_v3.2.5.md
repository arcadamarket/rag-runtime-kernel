# INIT_UNIVERSAL_RUNTIME_KERNEL_v3.2.5
...
> Supersedes: all prior versions through v3.2.4.
>
...
| init_prompt | INIT_UNIVERSAL_RUNTIME_KERNEL_v3.2.5.md | This specification |
...
1. **Checkpoint** RAG_MASTER.json — stamp `meta.written_by_session`, bump `last_checkpoint_seq`, mirror `.bak` to byte-parity.
2. **Close the logger** — gated on this session having checkpointed first (KA-4): a session cannot close unless its own checkpoint stamped `written_by_session`. This is the structural defense against the close-without-checkpoint freeze.
3. **Session-close audit** (§17/§18), fail-loud.

When the wrapper is present, `rag_kernel session-end` runs checkpoint → close → audit in that order; any step's failure aborts the rest. AUTONOMOUS mode performs the steps manually, in order.
...
```rag-config
{
  "operating_protocol": {
    ...
    "session_end_protocol": "HARD RULE — SESSION-END RITUAL. At session close, in order: (1) CHECKPOINT RAG_MASTER.json (stamp meta.written_by_session, bump last_checkpoint_seq, mirror .bak to byte-parity). (2) CLOSE the logger — GATED on this session having checkpointed first (KA-4): a session cannot close unless its own checkpoint stamped written_by_session; structural defense against the close-without-checkpoint freeze. (3) SESSION-CLOSE AUDIT (§17/§18), fail-loud. Runtime v0.4.14+: ONE governed command `rag_kernel session-end` runs checkpoint -> close -> audit, any step's failure aborts the rest. AUTONOMOUS mode performs the steps manually, in order. Pairs with session_start_protocol so each session boundary is a single ordered fail-loud ritual seeded into every fresh RAG (KA-10 TierB), not a per-project hand-script."
  }
}
```
...
END OF INIT_UNIVERSAL_RUNTIME_KERNEL_v3.2.5